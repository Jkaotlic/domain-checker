...
/**
 * Public DNS resolvers for fallback when system DNS fails or blocks queries.
 */
...
/**
 * Resolve both A + AAAA records with fallback resolvers.
 */
export async function resolveAllIPs(host: string, timeoutMs?: number): Promise<{ v4: string[]; v6: string[] }> {
  const [v4, v6] = await Promise.all([
    resolve4WithFallback(host, timeoutMs),
    resolve6WithFallback(host, timeoutMs),
  ]);
  return { v4, v6 };
}
